...
public interface UserRepository extends JpaRepository<User, Long> {
    ...
    List<User> findAllByAuthoritiesIn(String authority);  // 권한과 Role의 역할을 겸하고 있어서 따로 메서드 작성

    @Query("select a from User a, Authority b where a.userId=b.userId and b.authority=?1")
    Page<User> findAllByAuthoritiesIn(String authority, Pageable pageable);  // 목록들을 페이징된 리스트로 얻기
